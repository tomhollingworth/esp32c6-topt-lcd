...
{          
  ...
  uint32_t totalPixels = (uint32_t)Show_Width * (uint32_t)Show_Height;
  uint8_t dummy[64 * 2];
  ...
  while (remaining > 0) {
    ...
    LCD_WriteData_nbyte((uint8_t *)(color + offset), dummy, batch * sizeof(uint16_t));
    remaining -= batch;
    offset += batch;
  }
}
...
{
  const uint32_t totalPixels = (uint32_t)LCD_WIDTH * (uint32_t)LCD_HEIGHT;
  uint16_t line[64];
  uint8_t dummy[64 * 2];
  ...
  while (remaining > 0) {
    uint32_t batch = (remaining > 64) ? 64 : remaining;
    LCD_WriteData_nbyte((uint8_t *)line, dummy, batch * sizeof(uint16_t));
    remaining -= batch;
  }
}
// backlight
