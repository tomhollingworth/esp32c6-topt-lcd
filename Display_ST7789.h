#pragma once
#include <Arduino.h>
#include <SPI.h>
#define LCD_ROTATION 90 // 0, 90, 180, 270

#if (LCD_ROTATION == 0) || (LCD_ROTATION == 180)
	#define LCD_WIDTH   172 //LCD width
	#define LCD_HEIGHT  320 //LCD height
	#define Offset_X 34
	#define Offset_Y 0
#else
	#define LCD_WIDTH   320 //LCD width
	#define LCD_HEIGHT  172 //LCD height
	#define Offset_X 0
	#define Offset_Y 34
#endif

#define SPIFreq                        80000000
#define EXAMPLE_PIN_NUM_MISO           5
#define EXAMPLE_PIN_NUM_MOSI           6
#define EXAMPLE_PIN_NUM_SCLK           7
#define EXAMPLE_PIN_NUM_LCD_CS         14
#define EXAMPLE_PIN_NUM_LCD_DC         15
#define EXAMPLE_PIN_NUM_LCD_RST        21
#define EXAMPLE_PIN_NUM_BK_LIGHT       22
#define Frequency       1000     
#define Resolution      10       

#define VERTICAL   0
#define HORIZONTAL 1

#define LCD_MADCTL_BGR 0x00

#if (LCD_ROTATION == 0)
	#define LCD_MADCTL (0x00 | LCD_MADCTL_BGR)
#elif (LCD_ROTATION == 90)
	#define LCD_MADCTL (0x60 | LCD_MADCTL_BGR)
#elif (LCD_ROTATION == 180)
	#define LCD_MADCTL (0xC0 | LCD_MADCTL_BGR)
#elif (LCD_ROTATION == 270)
	#define LCD_MADCTL (0xA0 | LCD_MADCTL_BGR)
#else
	#define LCD_MADCTL (0x00 | LCD_MADCTL_BGR)
#endif


void LCD_SetCursor(uint16_t x1, uint16_t y1, uint16_t x2,uint16_t y2);

void LCD_Init(void);
void LCD_Clear(uint16_t color);
void LCD_SetCursor(uint16_t Xstart, uint16_t Ystart, uint16_t Xend, uint16_t  Yend);
void LCD_addWindow(uint16_t Xstart, uint16_t Ystart, uint16_t Xend, uint16_t Yend,uint16_t* color);

void Backlight_Init(void);
void Set_Backlight(uint8_t Light);
